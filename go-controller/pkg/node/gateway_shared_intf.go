...
func ensureMasqueradeResources(routeManager *routemanager.Controller, gwIface, nodeName string, watchFactory factory.NodeWatchFactory) error {
	...
	klog.Infof("ensureMasqueradeResources: restoring resources on %s (ifindex=%d, lastSuccessful=%d)",
	...
	klog.Infof("ensureMasqueradeResources: completed successfully for %s (ifindex=%d)", gwIface, lastMasqueradeLinkIndex)
	return nil
}
