...
func (g *gateway) runMasqueradeReconciler(stopChan <-chan struct{}, wg *sync.WaitGroup) {
	...
	go func() {
		defer wg.Done()
		timer := time.NewTicker(masqueradeReconcilePeriod)
		defer timer.Stop()
		klog.Infof("Starting masquerade reconciliation loop (period: %v)", masqueradeReconcilePeriod)
		for {
			select {
			case <-stopChan:
				klog.Info("Stopping masquerade reconciliation loop")
				return
			case <-timer.C:
				if err := g.doMasqueradeReconcile(); err != nil {
					klog.Errorf("Failed to reconcile masquerade: %v", err)
				}
			}
		}
	}()
